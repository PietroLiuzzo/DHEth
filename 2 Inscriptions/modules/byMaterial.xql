...
let $materials := distinct-values($dataset/ancestor::t:TEI//t:material/@ref)
let $materialLabels := for $m in $materials return  doc(concat($config:data-root, '/EAGLEvoc/eagle-vocabulary-material.rdf'))//skos:Concept[@rdf:about = $m]/skos:prefLabel/text()
let $headings := ('materials', $materialLabels)
...
let $rows := for $item in $items
                         let $counts := for $mat in $materials
                                                       let $selector :=  '$dataset/ancestor::t:TEI[descendant::t:' || $element || '[@ref=$item("uri")]][descendant::t:material[@ref=$mat]]'
                                                       let $selection := util:eval($selector)
                                                        return
                                                        count($selection)
      ...
      {'google.charts.load("current", {packages:["corechart"]});
        ...
        function drawChart() {
         ...
         || ");
          ...
          var options = {
            title: '"||$name||" by material "||$datasetname||"',
       isStacked: true,
          height: 300,
...
<script type="text/javascript" src="https://www.gstatic.com/charts/loader.js"></script>
 ...
 <body>
    {
for $datasetname in ('Ethio','ISicily', 'trismegistos', 'EAGLE')
