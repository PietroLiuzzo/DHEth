...
return 
<pair  xmlns="http://local.local">
    <name>{$kG2Name}</name>
    <tot>{string($t2)}</tot>
    <D>{$attestationsDelta}</D>
    <K>{$distance}</K>
</pair>
...
let $vectorX := number(replace(string(min($pairs//local:D)), '-', '')) 
let $vectorY := number(replace(string(min($pairs//local:K)), '-', '')) 
...
let $Subjects := doc(concat($config:data-rootA, '/taxonomy.xml'))//t:category[t:desc='Subjects']//t:category/t:catDesc[not(.='Translation')][not(contains(.,'Literature'))]

let $Periods := doc(concat($config:data-rootA, '/taxonomy.xml'))//t:category[t:desc='Periods']//t:category/t:catDesc

let $DatedWorks := collection($config:data-rootW)//t:term[@key = $Periods]
...
<work><uri>{'http://betamasaheft.eu/'||$id}</uri><title>{$id}</title></work>}
...
let $edges := (
local:compareGroups($groups, 'Aks', 'Paks1'), 
